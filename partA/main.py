...
def buildModel(sizeFilters,nFilters,activationFuncs,sizeMaxpool,sizeDenseLayers, dropout, _globalLayer,filterArrangement,batchNormalization, dataAugment = True):
    ...
    for i in range(convLayerSize):
      model.add(layers.Conv2D(nFilters[i], (sizeFilters[i], sizeFilters[i]), padding = "same"))
...
sweep_config = {
    ...
    'parameters' : {
        ...
        'filterSize' : {'values' : [5, 7]},
        'activationFuncs' : {'values' : ['sigmoid', 'tanh', 'relu']},
        'sizeMaxpool' : {'values' : [2, 3]},
        'nDense' :  {'values' : [ 64, 128, 512]},
        'dataAugment' :  {'values' : [True, False]},
        ...
        'convLayerSize' : {'values' : [5]},   #Always 5
        'denseLayerSize' : {'values' : [1]},   #Always 1
        } 
    }
'''
    sweep_config = {
